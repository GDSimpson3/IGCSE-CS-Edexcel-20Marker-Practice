...
FastestParticipant = ["NAME",19,0,0,0,float('inf')] #Total time wise - ["Name",age,swim,cycle,run, TotalTime] hence check via Fastest[5]
...
while count < MaxParticipants:
    ...
    try: 
        ...
        if age >= MinAge:
            ...
            if totalTime < FastestParticipant[5]: # Lower the score, faster the participant
...
print(f"\n {FastestParticipant[0]} (aged {FastestParticipant[1]}) Was the Fastest!!, Swimming: {FastestParticipant[2]}, Cycling: {FastestParticipant[3]}, Running: {FastestParticipant[4]}, With a total Score of {(FastestParticipant[2] + FastestParticipant[3] + FastestParticipant[4])} ")
